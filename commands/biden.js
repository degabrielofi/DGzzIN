...
exports.run = async (client, message, args) => {
  let degabrielofi = new Discord.MessageEmbed()
    .setAuthor("COMANDO: BIDEN", "https://i.imgur.com/0b6Ohrl.png")
    ...
    .addFields(
      {
        ...
        value:
          "Utilize este comando para que o Biden fale algo. Escreva com `d$biden (Mensagem).`",
        inline: true,
      },
      {
      ...
      },
      {
        ...
        value: "`d$biden Life is a matter of really hard choices`",
        inline: true,
      }
    );

  let img = jimp.read("https://i.imgur.com/PwARJKL.jpg");
  if (!args[0])
  ...
  img.then((image) => {
    jimp.loadFont(jimp.FONT_SANS_32_BLACK).then((font) => {
      ...
      image.getBuffer(jimp.MIME_PNG, (err, i) => {
        message.reply({
          content: `${message.author}`,
          files: [{ attachment: i, name: "biden.png" }],
        });
      });
    });
  });
