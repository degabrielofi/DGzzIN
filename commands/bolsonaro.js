...
exports.run = async (client, message, args) => {
  let degabrielofi = new Discord.MessageEmbed()
    .setAuthor("COMANDO: BOLSONARO", "https://i.imgur.com/0b6Ohrl.png")
    ...
    .addFields(
      {
        ...
        value:
          "Utilize este comando para que o Bolsonaro fale algo. Escreva com `d$bolsonaro (Mensagem).`",
        inline: true,
      },
      {
        name: "<:Sinonimos:1214053417933340692> Sinônimos:",
        value: "`d$bolsonaro` `d$bolso`",
        inline: true,
      },
      {
        name: "\u200b",
      ...
      },
      {
        ...
        value: "`d$bolsonaro TÁ OK `",
        inline: true,
      }
    );

  let img = jimp.read("https://i.imgur.com/9COEiZv.png");
  if (!args[0])
  ...
  img.then((image) => {
    jimp.loadFont(jimp.FONT_SANS_32_BLACK).then((font) => {
      ...
      image.getBuffer(jimp.MIME_PNG, (err, i) => {
        message.reply({
          content: `${message.author}`,
          files: [{ attachment: i, name: "bolsonaro.png" }],
        });
      });
    });
  });
